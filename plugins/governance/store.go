	...
	for entity, rateLimits := range entityWiseRateLimits {
		for _, rateLimit := range rateLimits {
			...
			if rateLimit.TokenResetDuration != nil {
				if res, err := EvaluateQuotaWindow(now, *rateLimit.TokenResetDuration, rateLimit.TokenLastReset, rateLimit.IsCalendarAligned); err != nil {
					gs.logger.Warn("CheckRateLimit: invalid token reset duration for %s: %v", rateLimit.ID, err)
				} else if res.PeriodEnded {
					tokenLimitExpired = true
				}
			}
			requestLimitExpired := false
			if rateLimit.RequestResetDuration != nil {
				if res, err := EvaluateQuotaWindow(now, *rateLimit.RequestResetDuration, rateLimit.RequestLastReset, rateLimit.IsCalendarAligned); err != nil {
					gs.logger.Warn("CheckRateLimit: invalid request reset duration for %s: %v", rateLimit.ID, err)
				} else if res.PeriodEnded {
...
func (gs *LocalGovernanceStore) CheckBudget(ctx context.Context, entityWiseBudgets EntityWiseBudgets, baselines map[string]float64) (Decision, error) {
	...
	for entity, budgets := range entityWiseBudgets {
		for _, budget := range budgets {
			if res, err := EvaluateQuotaWindow(now, budget.ResetDuration, budget.LastReset, budget.IsCalendarAligned); err != nil {
				gs.logger.Warn("CheckBudget: invalid reset duration for %s: %v", budget.ID, err)
			} else if res.PeriodEnded {
			...
			gs.logger.Debug("LocalStore CheckBudget: Checking %s budget %s: local=%.4f, remote=%.4f, total=%.4f, limit=%.4f",
				entity, budget.ID, budget.CurrentUsage, baseline, spent, budget.MaxLimit)
...
func (gs *LocalGovernanceStore) ResetExpiredBudgetsInMemory(ctx context.Context) []*configstoreTables.TableBudget {
	...
	gs.budgets.Range(func(key, value any) bool {
		...
		res, err := EvaluateQuotaWindow(now, budget.ResetDuration, budget.LastReset, calendarAligned)
		if err != nil {
		...
		resetBudget, ok := gs.ResetBudgetAt(ctx, budget.ID, res.NewLastReset)
		if !ok {
		...
		if gs.redisAvailable.Load() && gs.redisCounters != nil {
			var ttl int64
			if resetBudget.ResetDuration != "" {
				if dur, err := configstoreTables.ParseDuration(resetBudget.ResetDuration); err == nil {
					ttl = int64(dur.Seconds())
				}
			}
			if err := gs.redisCounters.ResetBudget(ctx, resetBudget.ID, ttl); err != nil {
				gs.logger.Warn("redis ResetBudget failed for %s: %v", resetBudget.ID, err)
			}
		}
		return true
	})
...
func (gs *LocalGovernanceStore) ResetExpiredRateLimitsInMemory(ctx context.Context) []*configstoreTables.TableRateLimit {
	...
	gs.rateLimits.Range(func(key, value any) bool {
		...
		tokenNewLastReset := evaluateQuotaWindowPtr(now, rateLimit.TokenResetDuration, rateLimit.TokenLastReset, calendarAligned)
		requestNewLastReset := evaluateQuotaWindowPtr(now, rateLimit.RequestResetDuration, rateLimit.RequestLastReset, calendarAligned)
		if tokenNewLastReset == nil && requestNewLastReset == nil {
			return true
		}
		...
		gs.resetRateLimitRedis(ctx, resetRateLimit, tokenNewLastReset != nil, requestNewLastReset != nil)
		return true
	})
