...
// HTTP clients select a prompt via bf-prompt-id / bf-prompt-version headers; optional
...
// PromptStore is the data source for prompts and all versions. Implementations typically
...
// keys populated from HTTP headers in HTTPTransportPreHook (bf-prompt-id, bf-prompt-version).
...
// Init constructs a Plugin using the default header-based resolver (bf-prompt-id / bf-prompt-version).
//
// Parameters:
...
// HTTPTransportPreHook copies bf-prompt-id and bf-prompt-version from the incoming HTTP request
...
//
// Parameters:
//   - ctx: may set BifrostContextKeySelectedPromptName and BifrostContextKeySelectedPromptVersion when a prompt is applied
...
// parseStringFromCtx returns a trimmed string from context, or empty if missing or not a string.
