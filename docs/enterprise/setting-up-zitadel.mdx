...
<Frame>
  <img src="/media/user-provisioning/zitadel-new-application.png" alt="Creating a new application in Zitadel" />
</Frame>
...
## Step 2: Enable role claims on the project
...
2. Authorize users to the relevant roles via **Authorizations → New Authorization**.
...
<Frame>
  <img src="/media/user-provisioning/zitadel-bifrost-config.png" alt="Bifrost Zitadel configuration form" />
</Frame>
...
| `clientSecret` | No | Web Application secret. Omit for PKCE-only flows. |
| `projectId` | Recommended | Required to resolve project-scoped role claims and sync role grants. |
| `audience` | No | Override the expected JWT `aud` claim. |
| `serviceAccountClientId` | For provisioning | Service account used to list users via the Management API. |
| `serviceAccountClientSecret` | For provisioning | Service account secret (shown once in Zitadel). |
| `teamIdsField` | No | JWT claim for team/group IDs. Defaults to `groups`. |
...
2. You'll be redirected to Zitadel. Sign in with a user that has a project authorization.
