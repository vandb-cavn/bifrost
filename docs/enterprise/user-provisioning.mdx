...
| `userIdField` | Optional | JWT claim used as the unique user ID. |
| `rolesField` | Optional | JWT claim containing role values. |
| `teamIdsField` | Optional | JWT claim containing group or team IDs. Defaults to `groups` for most providers. |
...
| --- | --- |
| Redirect loop on login | Redirect URI mismatch. Exact string match required — check trailing slashes and `http` vs `https`. |
