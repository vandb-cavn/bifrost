...
Phase 4 → SSO                     (enterprise login)
Phase 5 → Automated Provisioning  (SCIM, enterprise lifecycle)
Phase 6 → Observability & Audit   (compliance floor)
```
...
**Business value:** Reduces friction for large teams (no password to manage), satisfies security teams (MFA enforced at IdP level), enables the SCIM provisioning in Phase 5.

---

### Feature 4.1 — Social SSO (Google & GitHub)
...
### Feature 4.2 — Enterprise SSO (OIDC — Okta, Azure Entra, Keycloak)
...
## Phase 5: Automated Provisioning (SCIM)
...
### Feature 5.1 — SCIM 2.0 User Provisioning
...
- SCIM user creation follows the same rules as local user creation: email must be unique, role is assigned by group mapping (same mapping configured in OIDC Phase 4.2).
...
**Business value:** Enables enterprise procurement to tick compliance checkboxes. Built last because Phases 1–5 generate the actors and events that audit logs need to be meaningful.

---

### Feature 6.1 — Audit Logs
...
Phase 1: Multi-user + RBAC
    ...
    │
    ├──→ Phase 4: SSO (layered on users table, same session model)
    │        │
    │        └──→ Phase 5: SCIM (requires OIDC group mapping from Phase 4.2)
    │
    └──→ Phase 6: Audit Logs (needs actors from Phase 1, events from Phases 2–5)
              │
              └──→ Phase 6.2: Log Exports (extends audit logs)
```
...
The clearest gap to own: **SCIM + user-level budgets + access profiles** combined. No competitor does all three.
