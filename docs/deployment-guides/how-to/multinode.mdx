...
<Info>
  **PostgreSQL + Redis (OSS cluster):** If all nodes share one Postgres `config_store` and you configure the optional `cluster` block (Redis Streams + shared counters), configuration and governance counters can stay consistent across nodes without sharing a single `config.json` file. See [Cấu hình cluster OSS (Redis)](/deployment-guides/how-to/oss-cluster-config) (Vietnamese guide with field reference).
</Info>

<Warning>
  **Postgres `config_store` on multiple OSS nodes:** If each node has its own database or you do **not** use the shared **`cluster`** Redis settings, in-memory configuration and governance state will **diverge** between nodes. Use either (1) a **shared `config.json`** without `config_store` (this page), or (2) **one shared Postgres** plus **`cluster.redis`** — see [Cấu hình cluster OSS (Redis)](/deployment-guides/how-to/oss-cluster-config).

  Enterprise deployments can use **P2P clustering** (gossip) on top of the database; see [Clustering](/enterprise/clustering) for that product line.
</Warning>
...
|--------|--------------|---------------------|
| **Configuration Source** | Shared `config.json`, *or* shared Postgres + optional `cluster` (Redis) | Database with P2P sync |
| **Sync Mechanism** | File sharing; or Redis Streams + DB for OSS `cluster` | Gossip protocol (real-time) |
| **Config Updates** | Modify file + restart; or UI/API with stream-driven reload on other nodes (OSS `cluster`) | UI/API with automatic propagation |
...
<Note>
  If you use **one shared Postgres** and the **`cluster`** block with Redis, UI/API updates can propagate to other nodes — see [Cấu hình cluster OSS (Redis)](/deployment-guides/how-to/oss-cluster-config). The synchronization problem below applies when **that** setup is **not** used.
</Note>
...
| Multinode OSS | Shared `config.json` without `config_store`, **or** shared Postgres + `cluster` + Redis ([guide](/deployment-guides/how-to/oss-cluster-config)) |
